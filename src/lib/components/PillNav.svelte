...
<script lang="ts">
	...
	type Props = {
		...
		onLogoClick?: () => void;
		onItemClick?: (href: string, index: number, event: MouseEvent) => void;
		...
		if (mobileMenuRef) {
			if (newState) {
				...
				gsap.to(mobileMenuRef, {
					...
					transformOrigin: "top center",
					onComplete: () =>
						gsap.set(mobileMenuRef, { visibility: "hidden" }),
				});
			}
		}
		onMobileMenuClick?.();
				...
				{#each items as item, i (item.href)}
					<li role="none">
						<a
							role="menuitem"
							href={item.href}
							class="pill {activeHref === item.href
								? 'is-active'
								: ''}"
		...
		<ul class="mobile-menu-list">
			{#each items as item, i (item.href)}
				<li>
					<a
						href={item.href}
						class="mobile-menu-link {activeHref === item.href
							? 'is-active'
							: ''}"
						onclick={(e) => {
							playNavClickSound(i);
...
<style>
	...
	:global(.pill-nav .desktop-only) {
		display: block;
	}
	:global(.pill-nav .mobile-only) {
		display: none;
	}
	@media (max-width: 768px) {
		:global(.pill-nav .desktop-only) {
