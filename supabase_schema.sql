...
CREATE TABLE IF NOT EXISTS portfolio_experience (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    type TEXT NOT NULL CHECK (type IN ('internship', 'achievement', 'experience')),
    title TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS portfolio_skills (
    ...
    category TEXT NOT NULL CHECK (category IN (
        'FRONTEND', 'BACKEND', 'DATABASE', 'DEVOPS', 'AWS',
        'GIT', 'MONITORING', 'ENVIRONMENT', 'DEVELOPER TOOLS'
    )),
    name TEXT NOT NULL,
    icon_path TEXT NOT NULL,
    icon_type TEXT NOT NULL DEFAULT 'svg' CHECK (icon_type IN ('svg', 'webp', 'png', 'url')),
...
CREATE POLICY "Public can view published home" ON portfolio_home FOR SELECT USING (status = 'published');
CREATE POLICY "Public can view published about" ON portfolio_about FOR SELECT USING (status = 'published');
CREATE POLICY "Public can view published education" ON portfolio_education FOR SELECT USING (status = 'published');
CREATE POLICY "Public can view published experience" ON portfolio_experience FOR SELECT USING (status = 'published');
CREATE POLICY "Public can view published skills" ON portfolio_skills FOR SELECT USING (status = 'published');
CREATE POLICY "Public can view published projects" ON portfolio_projects FOR SELECT USING (status = 'published' AND is_deleted = FALSE);
CREATE POLICY "Public can view active socials" ON portfolio_socials FOR SELECT USING (enabled = TRUE);
CREATE POLICY "Public can view active resume" ON resume_metadata FOR SELECT USING (is_active = TRUE);
